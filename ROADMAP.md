...
> **当前状态（2026-07-11）**：M1 ✅、M2 ✅、M3 ✅、M3.5 ✅（拼音容错强化+词库层，方案 [docs/M3.5-plan.md](docs/M3.5-plan.md)、原理 [docs/algorithm.md](docs/algorithm.md)）。四层架构落地：拼写变换统一框架（漏敲/多敲/句尾 partial）+ 白霜词库 37 万条（Viterbi 本地整句 62%，毫秒级/断网可用）+ LLM 整句 + 音节回验；自绘候选条 + 逐段确认。40 句含错测试集：首选 85%，句级覆盖 92%。下一步：M4 双模态融合；欠账：双拼、词库下载 UI（暂 CLI 编译）、真人录音测试集、M1 Air 基准、XPC 签名校验、IME 实机走查、LLM p50 涨至 0.97s 待优化。
...
- [x] 设置中心拼音页：模糊音矩阵 + Playground 调试区（双拼顺延 M3.5）
